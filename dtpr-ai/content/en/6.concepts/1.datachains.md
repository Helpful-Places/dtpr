...
```http
POST /api/v2/schemas/dtpr@2026-09-01-beta/resolve
...
```json
{
  ...
  "schema_version": "dtpr@2026-09-01-beta",
  "created_at": "2026-09-01T00:00:00Z",
  "elements": [ "/* the placements above, unchanged */" ],
  "schema_snapshot": {
...
```json
{
  "authoring_provenance": {
    ...
    "element_provenance": {
      "recommends": {
        "rationale": "Operator's privacy notice says the model ranks applicants and a recruiter makes the call.",
        "confidence": "high",
        "source_references": [
          {
            "quote": "The screener returns a shortlist; a recruiter decides who advances.",
            "context": "Privacy notice §2 — How results are used"
          }
        ],
        "variable_rationale": {
          "additional_description": "Recruiter sign-off is described in the same paragraph."
        }
      },
      "employment": {
...
- **Orphan keys are rejected.** Every key in `element_provenance` must reference either a placement's `element_instance_id` (preferred) or an `element_id` that is placed exactly once and carries no `element_instance_id`. Both `validate` and `validate_resolved` emit `element_provenance_unknown_element` for any miss, and `element_provenance_ambiguous` when the key matches an `element_id` placed more than once. The same key-scoping rule applies one level deeper to `variable_rationale`: every key inside the per-element variable-rationale map must reference a variable declared on the placed element's category, or the validators emit `variable_rationale_unknown_variable`.
- **Multi-placement keying via `element_instance_id`.** When the same `element_id` appears more than once on `elements[]` (canonical case: `institution` placed as both `deployer` and `vendor`), each placement needs its own `element_instance_id` so per-element provenance entries can target a specific placement rather than collapsing onto a shared key. `element_instance_id` is a stable string unique within `elements[]`, drawn from the same `[a-zA-Z0-9_-]+` alphabet as `subchain_instances[].id`. Single-placement disclosures continue to key by `element_id` and need no change.
