...
- **Orphan keys are rejected.** Every key in `element_provenance` must reference a placement `element_id` on this datachain. Both `validate` and `validate_resolved` emit `element_provenance_unknown_element` for any key that does not.
