...
- **Orphan keys are rejected.** Every key in `element_provenance` must reference either a placement's `element_instance_id` (preferred) or an `element_id` that is placed exactly once and carries no `element_instance_id`. The semantic validator emits `element_provenance_unknown_element` for any miss, and `element_provenance_ambiguous` when the key matches an `element_id` placed more than once.
