...
```
/api/v2/schemas/dtpr@2026-09-01-beta/elements/purpose.example/icon.svg          # default
/api/v2/schemas/dtpr@2026-09-01-beta/elements/purpose.example/icon.dark.svg     # dark
/api/v2/schemas/dtpr@2026-09-01-beta/elements/purpose.example/icon.commercial.svg  # context-colored
```
...
Icon routes are charged to their own bucket, **1200 requests per 60 s**, rather than to the 300/60 s bucket the JSON reads share. They are pre-baked SVG point-reads served with a long `Cache-Control`, and clients fetch them in whole-library sweeps — a full icon set is ~470 requests and clears inside one window. See [Errors → rate limiting](/rest/errors#rate-limiting) for the bucket table and the 429 envelope.
...
```bash [curl — symbol]
curl -s -i https://api.dtpr.io/api/v2/schemas/dtpr@2026-09-01-beta/symbols/example-symbol.svg
```

```bash [curl — composed]
curl -s -i https://api.dtpr.io/api/v2/schemas/dtpr@2026-09-01-beta/elements/purpose.example/icon.dark.svg
```
::
