...
| `element_provenance_unknown_element` | 200 | A key in `authoring_provenance.element_provenance` matches neither a placement's `element_instance_id` nor a uniquely-placed `element_id`. |
| `element_provenance_ambiguous` | 200 | A key in `authoring_provenance.element_provenance` matches an `element_id` placed more than once. Use `element_instance_id` on each placement and key the entry by that id. |
| `INSTANCE_ELEMENT_INSTANCE_ID_DUPLICATE` | 200 | Two placements share the same `element_instance_id`. Must be unique within `elements[]`. |
| `INSTANCE_ELEMENT_INSTANCE_ID_COLLIDES_WITH_ELEMENT_ID` | 200 | An `element_instance_id` matches an `element_id` placed on this instance. The two namespaces must be disjoint. |
