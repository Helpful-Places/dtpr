...
::callout{type="info"}
**[`dtpr-figma-plugin.zip` (v0.1.0)](/figma-plugin/0.1.0/dtpr-figma-plugin.zip)** — the built plugin: `manifest.json`, `dist/code.js`, `dist/ui.html`, and an install `README.txt`.
::
...
A full build is **468 icon requests** plus metadata, against an API that allows [300 reads per minute](/rest/errors). The plugin paces itself to stay inside that budget, identifies itself with a `DTPR-Client` header so it gets its own rate-limit bucket, honours `Retry-After` on a 429, and lets a single failed icon drop just that variant rather than the whole element.

Expect roughly **two minutes** for a full run. Two options cut it down when you only need a refresh:
