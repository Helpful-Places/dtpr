...
A symbol with id `camera` in `dtpr@2026-09-01-beta` may differ byte-for-byte from `camera` in the next release. Consumers that pin on `DTPR-Content-Hash` will see the change. Symbols are therefore stored under the version prefix in the content store; there is no cross-version "global" symbol namespace.
