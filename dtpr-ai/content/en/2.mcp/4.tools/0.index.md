...
Every tool returns the same [envelope](/mcp/envelope) shape (`ok`/`errors`/`meta`). Canonical schema version in all examples: `dtpr@2026-09-01-beta`.

| Tool | Purpose | Pagination | Soft-failure |
|------|---------|------------|--------------|
| [`list_schema_versions`](/mcp/tools/list-schema-versions) | Discover versions. | — | — |
...
- **Version argument** — every read tool takes `version: string` in canonical form `dtpr@YYYY-MM-DD[-beta]`, as returned by `list_schema_versions`. There are no aliases.
