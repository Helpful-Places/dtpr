...
MCP sibling to [POST /schemas/:version/resolve](/rest/resolve). Composes a thin `DatachainInstance` and the pinned schema's lean subset (datachain_type + referenced categories + referenced elements) into a single self-contained `ResolvedDatachain`.
