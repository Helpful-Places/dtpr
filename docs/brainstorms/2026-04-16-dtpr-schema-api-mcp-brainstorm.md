...
- R32. **CORS** is public: every route returns `Access-Control-Allow-Origin: *` with `credentials: false`, callable from any origin including third-party browser-side JavaScript. The API has no cookies, sessions, or per-caller secrets — throttling is enforced via the rate-limit middleware (R29), not CORS. (Earlier drafts of this requirement specified a restrictive allow-list; that was reversed once the API was confirmed to be intentionally public.)
