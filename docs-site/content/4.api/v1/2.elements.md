...
```json
"icon": {
  "url": "https://api.dtpr.io/api/v1/icons/identifiable_video.svg",
...
| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `url` | string | yes | Absolute URL of the icon file |
| `alt_text` | locale array | yes | Localized alternative text for accessibility |
| `format` | string | yes | File format (e.g., `"svg"`, `"png"`) |

`url` is absolute, so it can be fetched or bound to an `<img src>` as-is from any origin. Icons are namespaced per API version with no cross-version fallback, so a v1 icon id is not necessarily present under `/api/v0/icons/`.
