...
::callout{type="info"}
**Endpoints:**
- `GET /api/v1/elements` — All elements
- `GET /api/v1/elements/:datachain_type` — Elements by datachain type (`device`, `ai`)

Base URL: `https://api.dtpr.io`
::
...
```json
"icon": {
  "url": "/api/v1/icons/identifiable_video.svg",
...
| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `url` | string | yes | Path to the icon file, root-relative to the API base (`https://api.dtpr.io`) |
| `alt_text` | locale array | yes | Localized alternative text for accessibility |
| `format` | string | yes | File format (e.g., `"svg"`, `"png"`) |

Resolve `url` against the API base to fetch the file — `https://api.dtpr.io/api/v1/icons/identifiable_video.svg`. Icons are namespaced per API version with no cross-version fallback, so a v1 icon id is not necessarily present under `/api/v0/icons/`.
