...
| *(body)* | JSON | A `DatachainInstance`. Shape is published in the schema's `schema_json`. The optional `authoring_provenance` block is accepted on this endpoint — see [Authoring provenance](/concepts/datachains#authoring-provenance). |
